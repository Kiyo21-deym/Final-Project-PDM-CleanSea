...
void loadWasteCollectionHistory();
...
void loginAccount() {
    char inputEmail[100], inputPassword[50];
    printf("\n== Account Login ==\n");

    FILE *fp = fopen("users.txt", "r");
    char email[100], password[50], username[50];
    if (fp != NULL) {
        fscanf(fp, "Username : %[^\n]\n", username);
        fscanf(fp, "Email : %[^\n]\n", email);
        fscanf(fp, "Password : %[^\n]\n", password);
...
void adminInputSampah(struct user *u) {
    ...
    while(1){
        ...
        printf("Jenis Sampah: ");
        scanf(" %[^\n]", jenis_sampah); getchar();
        if(strcmp(jenis_sampah, "Kaca") == 0 ||
           strcmp(jenis_sampah, "Plastik") == 0 ||
           strcmp(jenis_sampah, "Kertas") == 0 ||
           strcmp(jenis_sampah, "Elektronik") == 0 ||
           strcmp(jenis_sampah, "Logam") == 0 ||
           strcmp(jenis_sampah, "Minyak Jelantah") == 0){
            printf("Berat Sampah (Kg): ");
            scanf("%f", &berat_sampah); getchar();
            break;
        }
        else {
    ...
    FILE *fp = fopen("riwayat_sampah.txt", "a");
    if (fp != NULL) {
...
void OutputSampah(int user_id) {
    ...
    while(fgets(line, sizeof(line), fp) != NULL){
        ...
        if(sscanf(line, "\"%[^\"]\" - %f Kg - \"%[^\"]\" - %d poin", jenis_sampah, &berat_sampah, lokasi, &poin_didapat) == 4){
            printf("%d. %-15s - %6.2f Kg - %-20s - %d poin\n", num++, jenis_sampah, berat_sampah, lokasi, poin_didapat);
        }
    }
    fclose(fp);
}
    ...
    do{
        ...
        switch (userChoice){
            case 1:
                ...
                scanf(" %[^\n]", currentUser.username); getchar();
                printf("Enter Email\t: ");
                scanf(" %s", currentUser.email); getchar();
                printf("Enter Password\t: ");
                scanf(" %[^\n]", currentUser.password); getchar();
                printf("Confirm Password: ");
                scanf(" %[^\n]", confirmPassword); getchar();
                registerAccount(currentUser.username, currentUser.email, currentUser.password, confirmPassword);
                break;
            case 2:
                loginAccount(currentUser.email, currentUser.password);
                break;
            case 3:
                displayGreeting(currentUser.username);
            ...
            case 4:
                ...
                break;
            case 5:
                loadRewardPage(&currentUser);
                break;
            case 6:
                adminScanQRCodeForSampah("QR_DATA_SAMPLE", &currentUser);
                break;
            case 7:
                loadCollectionLocations(&currentUser);
                break;
            case 8:
                loadWasteCollectionHistory();
                checkBadge(&currentUser);
                OutputSampah(currentUser.id);
                break;
            case 9:
            ...
            case 11:
                ...
                UpdateUserLevel(currentUser.total_kg);
                break;
            case 12:
                loadHelpMenu();
                loadFAQ();
